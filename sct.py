...
@click.option("-r", "--region", "regions", type=CloudRegion(), help="Cloud region to query images in", multiple=True)
@click.option(
    "-a",
    "--arch",
...
def show_log(test_id, output_format, update_argus: bool):
    ...
    if output_format == "table":
        ...
        click.echo(rich_table_to_string(table, title=f"Log links for testrun with test id {test_id}"))
...
def search_builder(test_id):
    ...
    click.echo(rich_table_to_string(tbl, title=f"Found builders for Test-id: {test_id}"))
...
def collect_logs(test_id=None, logdir=None, backend=None, config_file=None):
    ...
    if collector.test_id:
        store_logs_in_argus(
            test_id=UUID(collector.test_id), logs=collected_logs, use_tunnel=config.get("argus_use_ssh_tunnel") or False
        )
...
def send_email(
    ...
    client = init_argus_client(argus_test_id, use_tunnel=get_argus_use_tunnel_from_env())
    run = client.get_run()
...
def create_operator_test_release_jobs(branch, username, password, sct_branch, sct_repo, triggers):
    ...
    server.create_job_tree(
        f"{server.base_sct_dir}/jenkins-pipelines/operator",
...
def create_performance_jobs(username, password, sct_branch, sct_repo, triggers):
    ...
    server.create_job_tree(
        f"{server.base_sct_dir}/jenkins-pipelines/performance/{sct_branch}",
        create_freestyle_jobs=triggers,
        job_name_suffix="",
    )
...
def create_nemesis_pipelines(base_job: str, backend: list[str], diff: bool):
    ...
    gen = NemesisJobGenerator(base_job=base_job, backends=backend, base_dir="", file_opener=file_opener)
...
def create_runner_image(cloud_provider, region, availability_zone):
    ...
    sct_runner = get_sct_runner(
        cloud_provider=cloud_provider, region_name=region, availability_zone=availability_zone, params=sct_config
    )
...
def create_argus_test_run():
    try:
        ...
        if getattr(argus_client, "_replay_log_only", False):
            LOGGER.warning(
