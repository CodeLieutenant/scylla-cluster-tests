...
class CommandLog(BaseLogEntity):
    ...
    def collect(self, node, local_dst, remote_dst=None, local_search_path=None) -> Optional[str]:
        ...
        remote_logfile, is_file_remote = LogCollector.collect_log_remotely(
        ...
        if not ssh_connected and (ssm_runner := node.aws_ssm_runner):
            ...
            try:
                ...
                if not ssm_result.ok:
                    LOGGER.error(
                        "SSM command failed for instance %s: %s ", node._instance.get("InstanceId"), collect_log_command
                    )
...
class SirenManagerLogCollector(LogCollector):
    log_entities = [
        ...
        CommandLog(name="scylla.yaml", command=f"cat {SCYLLA_YAML_PATH}"),
    ]
...
class VectorStoreLogCollector(LogCollector):
    log_entities = [
        FileLog(name="system.log", command="sudo journalctl --no-tail --no-pager", search_locally=True),
        FileLog(
            name="vector_store.log", command="sudo journalctl -u vector-store.service --no-tail", search_locally=True
        ),
    ]
...
class Collector:
    ...
    def _add_xcloud_node_to_collecting_set(
        ...
        ssh_login_info = getattr(self.localhost, ssh_method_name)(
            node=XCloudNodeMock(
                _cluster_id=cluster_id, _node_id=node_id, parent_cluster=XCloudParentClusterMock(self.params)
            )
        )

        collecting_set.append(
            CollectingNode(name=str(node_id), ssh_login_info=ssh_login_info, tags={**self.tags, "NodeType": node_type})
        )
    ...
    def get_xcloud_instances_by_testid(self):
            ...
            try:
                ...
                for vs_node_data in vs_node_list:
                    self._add_xcloud_node_to_collecting_set(
                        ...
                        "vector-store",
                        self.vector_store_set,
                        "xcloud_connect_get_vs_ssh_address",
                    )
            except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            self._add_xcloud_node_to_collecting_set(
                ...
                "manager",
                self.siren_manager_set,
                "xcloud_connect_get_manager_ssh_address",
            )
        except Exception as exc:  # noqa: BLE001
...
class SchemaLogCollector(BaseSCTLogCollector):
    log_entities = [
        FileLog(name="schema.log", search_locally=True),
