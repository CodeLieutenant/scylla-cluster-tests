...
class CloudK8sNodePool(metaclass=abc.ABCMeta):
    ...
    def __str__(self):
        data = [
            f'name="{self.name}"',
            *[f'{param}="{value}"' for param, value in self.__dict__.items() if param not in ["name", "k8s_cluster"]],
        ]
...
class KubernetesCluster(metaclass=abc.ABCMeta):
    ...
    def kubectl_no_wait(
        self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, ignore_status=False, verbose=True
    ):
    ...
    def kubectl_wait(self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, verbose=True):
        ...
        def wait_body():
            ...
            result = self.kubectl(
                "wait --timeout=1m", *command, namespace=namespace, timeout=timeout, remoter=remoter, verbose=verbose
            )
    ...
    def kubectl_multi_cmd(
        ...
        if self.api_call_rate_limiter:
            self.api_call_rate_limiter.wait()
        return KubernetesOps.kubectl_multi_cmd(
            self,
            *command,
            namespace=namespace,
            timeout=timeout,
    ...
    def set_nodeselector_for_deployments(
        self, pool_name: str, namespace: str = "kube-system", selector: str = ""
    ) -> None:
        ...
        deployment_names = self.kubectl(
            f"get deployments -l '{selector}' --no-headers -o custom-columns=:.metadata.name", namespace=namespace
        ).stdout.split()
        for deployment_name in deployment_names:
            self.kubectl(f"patch deployments {deployment_name} -p '{json.dumps(data)}'", namespace=namespace)
    ...
    def deploy_cert_manager(self, pool_name: str = None) -> None:
        ...
        self.kubectl_wait("--all --for=condition=Ready pod", namespace="cert-manager", timeout=600)
        wait_for(
    ...
    def get_latest_chart_version(self, local_chart_path: str) -> str:
        all_versions = yaml.safe_load(self.helm(f"search repo {local_chart_path} --devel --versions -o yaml"))
        ...
        for version_object in all_versions:
            if version_object["version"].count("-") == 1:
                continue
            if ComparableScyllaOperatorVersion(version_object["version"]) > (current_newest_version or "0.0.0"):
    ...
    def scylla_operator_chart_version(self):
        self.log.debug(self.helm(f"repo add scylla-operator {self.params.get('k8s_scylla_operator_helm_repo')}"))
        ...
        if chart_version in ("", "latest"):
            ...
            self.log.info("Using automatically found following latest scylla-operator chart version: %s", chart_version)
        else:
            self.log.info("Using following predefined scylla-operator chart version: %s", chart_version)
    ...
    def get_operator_image(self, repo: str = "", chart_version: str = "") -> str:
        ...
        chart_info = self.helm(f"show chart {chart_name} --devel --repo {repo} --version {chart_version}")
    ...
    def deploy_scylla_manager(self, pool_name: str = None) -> None:
        ...
        if not self.params.get("reuse_cluster"):
            ...
            scylla_operator_repo_base = self.params.get("k8s_scylla_operator_docker_image").rsplit("/", 1)[0]
            if scylla_operator_repo_base:
                values.set("controllerImage.repository", scylla_operator_repo_base)

            scylla_operator_image_tag = self.params.get("k8s_scylla_operator_docker_image").split(":", 1)[-1]
    ...
    def deploy_scylla_operator(self, pool_name: str = None) -> None:
        if not self.params.get("reuse_cluster"):
            ...
            if pool_name:
                ...
                affinity_rules["webhookServerAffinity"]["nodeAffinity"] = affinity_rules["affinity"]["nodeAffinity"]
            values = HelmValues(**affinity_rules)
            if ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) > "1.3.0":
            ...
            if enable_tls == "true" and ComparableScyllaOperatorVersion(scylla_operator_version) >= "1.9.0":
                ...
                for flag in (
                    ...
                    patch_obj = [{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value": flag}]
                    patch_cmd = f"patch deployment scylla-operator --type=json -p='{json.dumps(patch_obj)}'"
                    self.kubectl(patch_cmd, namespace=SCYLLA_OPERATOR_NAMESPACE)

            KubernetesOps.wait_for_pods_readiness(
                kluster=self, total_pods=lambda pods: pods > 0, readiness_timeout=5, namespace=SCYLLA_OPERATOR_NAMESPACE
            )
    ...
    @log_run_info
    def upgrade_scylla_operator(self, new_helm_repo: str, new_chart_version: str, new_docker_image: str = "") -> None:
        self.log.info(
        ...
        if new_chart_version in ("", "latest"):
            ...
            self.log.info(
                "Using automatically found following latest scylla-operator upgrade chart version: %s",
                new_chart_version,
            )
        else:
            self.log.info("Using following predefined scylla-operator upgrade chart version: %s", new_chart_version)
        ...
        values = HelmValues(
            json.loads(self.helm("get values scylla-operator -o json", namespace=SCYLLA_OPERATOR_NAMESPACE))
        )
        ...
        if (
            ...
            and ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) >= "1.8.0"
        ):
            patch_cmd = (
                'patch deployment scylla-operator --type=json -p=\'[{"op": "add",'
                '"path": "/spec/template/spec/containers/0/args/-", '
    ...
    def check_scylla_cluster_sa_annotations(self, namespace: str = SCYLLA_NAMESPACE):
        ...
        raw_sa_data = self.kubectl(
            f"get sa {self.params.get('k8s_scylla_cluster_name')}-member -o yaml", namespace=namespace
        ).stdout
    ...
    def deploy_minio_s3_backend(self):
        ...
        self.kubectl_wait("-l app=minio --for=condition=Ready pod", timeout=600, namespace=MINIO_NAMESPACE)

    def get_scylla_cluster_helm_values(
        self, cpu_limit, memory_limit, pool_name: str = None, cluster_name=None
    ) -> HelmValues:
        mgmt_agent_cpu_limit = convert_cpu_units_to_k8s_value(SCYLLA_MANAGER_AGENT_RESOURCES["cpu"])
        mgmt_agent_memory_limit = convert_memory_units_to_k8s_value(SCYLLA_MANAGER_AGENT_RESOURCES["memory"])
        if not cluster_name:
        ...
        if (
            self.params.get("k8s_enable_sni")
            and ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) >= "1.8.0"
        ):
        ...
        return HelmValues(
            {
                ...
                "alternator": {
                    ...
                    "insecureDisableAuthorization": not (self.params.get("alternator_enforce_authorization") or False),
                    **(
                        {"writeIsolation": self.params.get("alternator_write_isolation")}
                        if self.params.get("alternator_write_isolation")
                        else {}
                    ),
                },
                "developerMode": False,
    ...
    def install_dynamic_local_volume_provisioner(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        if self.params.get("reuse_cluster"):
            return
        if not isinstance(node_pools, list):
            node_pools = [node_pools]
        ...
        config_modifiers = [
            affinity_modifier for current_pool in node_pools for affinity_modifier in current_pool.affinity_modifiers
        ]
    ...
    def prepare_k8s_scylla_nodes(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        ...
        if self.is_performance_tuning_enabled:
            ...
            self.kubectl_wait("--for condition=established crd/scyllaoperatorconfigs.scylla.scylladb.com")
    ...
    def deploy_ingress_controller(self, pool_name: str = None):
        ...
        self.kubectl_wait("--all --for=condition=Ready pod", namespace=INGRESS_CONTROLLER_NAMESPACE, timeout=306)
        self.start_haproxy_ingress_log_thread()

    @log_run_info
    def deploy_scylla_cluster(
        self,
        node_pool_name: str,
        namespace: str = SCYLLA_NAMESPACE,
        cluster_name: str = None,
        s3_provider_endpoint: str = None,
    ) -> None:
        ...
        if self.params.get("reuse_cluster"):
            try:
                ...
                raise RuntimeError("SCT_REUSE_CLUSTER is set, but target scylla cluster is unhealthy") from exc
        ...
        self.log.debug("Check Scylla cluster")
        self.kubectl("get scyllaclusters.scylla.scylladb.com", namespace=namespace)
        self.log.debug("Wait for %d secs before we start to apply changes to the cluster", DEPLOY_SCYLLA_CLUSTER_DELAY)
    ...
    def deploy_scylla_cluster_monitoring(
        ...
        for condition in ("Progressing=False", "Degraded=False", "Available=True"):
            self.kubectl_wait(
                f"--for='condition={condition}' scylladbmonitoring {cluster_name}", namespace=namespace, timeout=600
            )
    ...
    @log_run_info
    def deploy_loaders_cluster(self, node_pool_name: str = "", namespace: str = LOADER_NAMESPACE) -> None:
        ...
        self.calculated_loader_memory_limit = memory_limit = convert_memory_units_to_k8s_value(memory_limit)
    ...
    def create_secret_from_data(self, secret_name: str, namespace: str, data: dict, secret_type: str = "Opaque"):
        prepared_data = {
            key: base64.b64encode(json.dumps(value).encode("utf-8")).decode("utf-8") for key, value in data.items()
        }
    ...
    def create_secret_from_directory(
        self, secret_name: str, path: str, namespace: str, secret_type: str = "generic", only_files: List[str] = None
    ):
        files = [
            fname
            for fname in os.listdir(path)
            if os.path.isfile(os.path.join(path, fname)) and (not only_files or fname in only_files)
        ]
        cmd = f"create secret {secret_type} {secret_name} " + " ".join(
            [f"--from-file={fname}={os.path.join(path, fname)}" for fname in files]
        )
    ...
    def start_token_update_thread(self):
        ...
        wait_for(
            ...
            timeout=30,
            step=5,
            text="Wait for gcloud token",
            throw_exc=True,
            path=self.kubectl_token_path,
        )

    def start_scylla_pods_ip_change_tracker_thread(self):
        self.scylla_pods_ip_change_tracker_thread = ScyllaPodsIPChangeTrackerThread(self, self.scylla_pods_ip_mapping)
    ...
    def move_pods_to_new_node_pool(
        self,
        pod_objects: list[cluster.BaseNode],
        node_pool_name: str,
        cluster_name: str = "",
        ...
        scylla_cluster_info = yaml.safe_load(
            self.kubectl(
                f"get scyllaclusters.scylla.scylladb.com {cluster_name} -o yaml", namespace=cluster_namespace
            ).stdout
        )
        racks_info = scylla_cluster_info["spec"]["datacenter"]["racks"]
        total_pods = 0
        for i, rack in enumerate(racks_info):
            rack["placement"]["nodeAffinity"]["requiredDuringSchedulingIgnoredDuringExecution"]["nodeSelectorTerms"][0][
                "matchExpressions"
            ][0]["values"] = [node_pool_name]
            racks_info[i] = rack
            total_pods += int(rack["members"])
        data = {"spec": {"datacenter": {"racks": racks_info}}}
        self.kubectl(
            f"patch scyllaclusters.scylla.scylladb.com {cluster_name} --type merge -p '{json.dumps(data)}'",
        ...
        for pod_object in sorted(pod_objects, key=lambda x: x.name, reverse=True):
            ...
            pod_object.wait_for_pod_readiness(pod_readiness_timeout_minutes=pod_readiness_timeout_minutes)
    ...
    def scylla_config_map(self, namespace: str = SCYLLA_NAMESPACE) -> dict:
        with self.scylla_config_lock:
            try:
                config_map = (
                    self.k8s_core_v1_api.read_namespaced_config_map(name=SCYLLA_CONFIG_NAME, namespace=namespace).data
                    or {}
                )
                exists = True
            ...
            if exists:
                ...
                self.k8s_core_v1_api.create_namespaced_config_map(
                    namespace=namespace, body=V1ConfigMap(data=config_map, metadata={"name": SCYLLA_CONFIG_NAME})
                )
...
class BasePodContainer(cluster.BaseNode):
    ...
    def start_journal_thread(self):
        self._journal_thread = get_system_logging_thread(
            logs_transport="k8s_client", node=self, target_log_file=self.system_log
        )
        if self._journal_thread:
            self.k8s_cluster.log.info("Use %s as logging daemon", type(self._journal_thread).__name__)
    ...
    def _pod(self):
        pods = KubernetesOps.list_pods(
            self.k8s_cluster, namespace=self.parent_cluster.namespace, field_selector=f"metadata.name={self.name}"
        )
    ...
    def private_ip_address(self) -> Optional[str]:
        if (
            ip := self.k8s_cluster.scylla_pods_ip_mapping.get(self.parent_cluster.namespace, {})
            .get(self.name, {})
            .get("current_ip")
        ):
            return ip
    ...
    def wait_till_k8s_pod_get_uid(self, timeout: int = None, ignore_uid=None, throw_exc=False) -> str:
        ...
        wait_for(
            lambda: self.k8s_pod_uid and self.k8s_pod_uid != ignore_uid,
            timeout=timeout,
    ...
    def wait_for_k8s_node_readiness(self):
        wait_for(
            self._wait_for_k8s_node_readiness,
    ...
    def terminate_k8s_node(self):
        ...
        """
        self.k8s_cluster.kubectl(f"delete node {self.node_name} --now", timeout=self.pod_terminate_timeout * 60 + 10)
...
class BaseScyllaPodContainer(BasePodContainer):
    ...
    def network_interfaces(self):
        interfaces = [
            NetworkInterface(
                ipv4_public_address=self.public_ip_address,
    ...
    def start_scylla_server(self, verify_up=True, verify_down=False, timeout=500, verify_up_timeout=None):
        ...
        self.remoter.run('sh -c "supervisorctl start scylla || supervisorctl start scylla-server"', timeout=timeout)
        if verify_up:
    ...
    @retrying(
        n=3,
        sleep_time=5,
        allowed_exceptions=NETWORK_EXCEPTIONS + (CommandTimedOut,),
    ...
    def stop_scylla_server(self, verify_up=False, verify_down=True, timeout=300, ignore_status=False):
        ...
        self.remoter.run(
            'sh -c "supervisorctl stop scylla || supervisorctl stop scylla-server"',
            timeout=timeout,
            ignore_status=ignore_status,
        )
        if verify_down:
    ...
    def restart_scylla_server(self, verify_up_before=False, verify_up_after=True, timeout=1800, verify_up_timeout=None):
        ...
        if verify_up_before:
            self.wait_db_up(timeout=verify_up_timeout)
        self.remoter.run('sh -c "supervisorctl restart scylla || supervisorctl restart scylla-server"', timeout=timeout)
        if verify_up_after:
            self.wait_db_up(timeout=verify_up_timeout)
    ...
    def _wait_for_svc(self):
        self.k8s_cluster.kubectl(f"get svc {self.name}", namespace=self.parent_cluster.namespace, verbose=False)
        return True
    ...
    def fstrim_scylla_disks(self):
        ...
        self.k8s_cluster.kubectl(f"exec -ti {podnames[0]} -- sh -c 'fstrim -v {scylla_disk_path}'", namespace=namespace)
    ...
    def alternator_ca_bundle_path(self):
        ...
        if ca_bundle_cmd_output.failed:
            self.k8s_cluster.log.warning("Failed to get alternator CA bundle info: %s", ca_bundle_cmd_output.stderr)
            return None
...
class PodCluster(cluster.BaseCluster):
    ...
    def add_nodes(
        ...
        self.wait_for_pods_running(pods_to_wait=count, total_pods=expected_dc_nodes_count, dc_idx=dc_idx)
        self.wait_for_pods_readiness(pods_to_wait=count, total_pods=expected_dc_nodes_count, dc_idx=dc_idx)
    ...
    def wait_sts_rollout_restart(self, pods_to_wait: int, dc_idx: int = 0):
        ...
        for statefulset in KubernetesOps.list_statefulsets(k8s_cluster, namespace=self.namespace):
            k8s_cluster.kubectl(
                f"rollout status statefulset/{statefulset.metadata.name} --watch=true --timeout={timeout}m",
    ...
    def wait_for_pods_readiness(
        self, pods_to_wait: int, total_pods: int, readiness_timeout: int = None, dc_idx: int = 0
    ):
        KubernetesOps.wait_for_pods_readiness(
...
class ScyllaPodCluster(cluster.BaseScyllaCluster, PodCluster):
    def __init__(
        ...
        for k8s_cluster in k8s_clusters:
            ...
            k8s_cluster.deploy_scylla_cluster(
                node_pool_name=node_pool_name, namespace=self.namespace, cluster_name=self.scylla_cluster_name, **kwargs
            )
        super().__init__(
    ...
    def scylla_manager_auth_token(self) -> str:
        raise RuntimeError("K8S uses different approach for setting up the scylla manager auth token")
    ...
    def refresh_scylla_pod_ip_address(self, pod_name):
        ...
        for node in self.nodes:
            ...
            LOGGER.warning("Could not find a node with the '%s' name in '%s' namespace", pod_name, self.namespace)
    ...
    def wait_for_nodes_up_and_normal(
        self, nodes=None, verification_node=None, iterations=None, sleep_time=None, timeout=None
    ):
        ...
        for dc_idx, dc_nodes in dc_node_mapping.items():
            self.wait_for_pods_readiness(
                pods_to_wait=len(dc_nodes),
                total_pods=len([n for n in self.nodes if n.dc_idx == dc_idx]),
    ...
    @timeout_wrapper(
        timeout=300,
        sleep_time=3,
        allowed_exceptions=NETWORK_EXCEPTIONS + (ClusterNodesNotReady,),
    ...
    def _k8s_scylla_cluster_api(self, dc_idx: int = 0) -> Resource:
        return KubernetesOps.dynamic_api(
            self.k8s_clusters[dc_idx].dynamic_client, api_version=SCYLLA_API_VERSION, kind=SCYLLA_CLUSTER_RESOURCE_KIND
        )

    @retrying(
        n=20,
        sleep_time=3,
        allowed_exceptions=(k8s_exceptions.ApiException,),
        message="Failed to update ScyllaCluster's spec...",
    )
    def replace_scylla_cluster_value(self, path: str, value: Any, dc_idx: int = 0) -> Optional[ANY_KUBERNETES_RESOURCE]:
    ...
    def get_scylla_cluster_plain_value(self, path: str, dc_idx: int = 0) -> Union[Dict, List, str, None]:
        ...
        """
        cluster_data = (
            self._k8s_scylla_cluster_api(dc_idx=dc_idx)
            .get(namespace=self.namespace, name=self.scylla_cluster_name)
            .to_dict()
        )
        return walk_thru_data(cluster_data, path)
    ...
    def node_config_setup(self, client_encrypt=None):
    ...
    def add_nodes(
        ...
        new_nodes = []
        self.log.debug(
            "'%s' configuration was taken for the 'dc_idx': %s", "Single-DC" if len(dc_idx) < 2 else "Multi-DC", dc_idx
        )
        for current_dc_idx in dc_idx:
            ...
            if current_dc_idx > 0:
                ...
                if not dc_podip_mapping.get(current_dc_idx, []):
                    ...
                    self.replace_scylla_cluster_value("/spec/externalSeeds", dc_podip_mapping[0], dc_idx=current_dc_idx)
            ...
            if self.params.get("use_mgmt") and current_dc_idx == 0 and (self.k8s_scylla_manager_auth_token is None):
                self.k8s_scylla_manager_auth_token = (
                    base64.b64decode(
                        kubectl(
                ...
                existing_k8s_scylla_manager_auth_token = (
                    base64.b64decode(
                        kubectl(
                            f"get secrets/{self.scylla_cluster_name}-auth-token"
                            " --template='{{ index .data \"auth-token.yaml\" }}'",
                ...
                if existing_k8s_scylla_manager_auth_token != self.k8s_scylla_manager_auth_token:
                    auth_token_base64 = base64.b64encode(self.k8s_scylla_manager_auth_token.encode("utf-8")).decode(
                        "utf-8"
                    )
                    patch_cmd = (
    ...
    def decommission(self, node: BaseScyllaPodContainer, timeout: int | float = None):
        ...
        shutil.move(
            node.system_log, os.path.join(node.logdir, f"system_{datetime.now().strftime('%y_%m_%d_%H_%M_%S')}.log")
        )
    ...
    def restart_scylla(self, nodes=None, random_order=False):
        ...
        for i, k8s_cluster in enumerate(self.k8s_clusters):
            ...
            if not current_k8s_cluster_nodes:
                self.log.warning("No DB nodes specified for the restart in the '%s' K8S cluster", k8s_cluster.name)
                continue
    ...
    def prefill_cluster(self, dataset_name: str):
        ...
        if not test_data:
            raise ValueError(
                f"Dataset unexpected value: '{dataset_name}'. "
...
class LoaderPodCluster(cluster.BaseLoaderSet, PodCluster):
    def __init__(
        ...
        if self.k8s_loader_run_type == "static":
            ...
            raise ValueError("'k8s_loader_run_type' has unexpected value: %s" % self.k8s_loader_run_type)
        PodCluster.__init__(
            self,
    ...
    def node_setup(self, node: BasePodContainer, verbose: bool = False, **kwargs) -> None:
    ...
    def add_nodes(
        ...
        for current_dc_idx in dc_idx:
            ...
            node_count_in_dc = count[current_dc_idx] if current_dc_idx < len(count) else count[0]
            k8s_loader_cluster_name = f"{self.loader_cluster_name}-{self.k8s_clusters[current_dc_idx].region_name}"
            ...
            self.k8s_clusters[current_dc_idx].log.debug(
                "Check the '%s' loaders cluster in the '%s' namespace", self.loader_cluster_name, self.namespace
            )
